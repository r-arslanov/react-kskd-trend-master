        ...
        if(this.state.needUpdate){
            Rest.getObjects(this.state.kust, this.state.type, response => {
                ...
                response.dps.forEach((dp, index)=>{
                    if(this.state.type === "SK_AI"){
                        arr_dps.push(this.state.kust + ":" + dp.dp);
                    }else{
                ...
                Rest.getGroup(arr_dps, (res2)=>{
                    ...
                    res2.forEach((gp)=>{
                        let tmp;
                        if(this.state.type === "SK_AI"){
                            tmp = gp.dp.replace(this.state.kust + ":", "");
                        }else{
                            tmp = gp.dp.replace(this.state.kust + ":" + this.state.kust + "=" + this.state.type + "_1.", "");
                        }
    ...
    render(){
        ...
        return(
            <div className="ControlDiv" id={"sidebar-div"}>
                ...
                <div className='objects-tags' id='objects-tag-div'>
                    {(this.state.objects.length !== 0) ? <ObjList data={this.state.objects} onChange={(value) => this.updObject(value)}/> : ""}
                    {(this.state.objects.length !== 0 || (this.state.type == "SK_AI" && this.state.dps.length !== 0)) ? <DpList data={this.state.dps} onChange={(key) => this.updSelected(key)} /> : ""}
                </div>
            </div>
        )
    }
